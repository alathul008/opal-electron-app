...
import { useStudioSetting } from '@/hooks/useStudioSettings'
...
const activeAudio=state.audioInputs?.find((device)=>device.deviceId===user?.studio?.mic)
    const {isPending,onPreset,register}=useStudioSetting(
        user!.id,
  ...
  return <form className='flex h-full relative w-full flex-col gap-y-5'>
    {isPending && (<div className='fixed z-50 w-full top-0 left-0 right-0 h-96 rounded-2xl bg-black/80 flex justify-center items-center'>
    <Loader/></div>)}
